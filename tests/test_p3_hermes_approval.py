...
from feishu_shadow_agent.processing import (
    ApprovalService,
    SendComposer,
    ...
    def task_session(
        ...
        *,
        ...
        return AgentRunResult(
            ["hermes"], 0, json_data=output, session_id=agent_session_id
        )
...
class FakeFeishu:
    ...
    def list_p2p_messages(
        self, *, user_id: str, page_token: str | None = None, **kwargs: Any
    ) -> MessagePage:
...
def _postprocess_config(
    *, profile_path: str = "owner_style.md"
) -> ReplyPostprocessConfig:
...
def _service(
    ...
    *,
    ...
    agent_working_dir = resolve_agent_working_dir(
        cfg.agent_backend.working_dir, base_dir
    )
...
def test_gate_passed_p2p_creates_pending_send_and_persists_session(
    tmp_path: Path,
) -> None:
    hermes = FakeHermes()
...
def test_new_task_stores_configured_agent_working_dir(tmp_path: Path) -> None:
    ...
    store, service, _ = _service(
        tmp_path, config=cfg, hermes=hermes, config_base_dir=tmp_path
    )
...
def test_existing_task_followup_uses_stored_agent_working_dir_after_config_change(
    tmp_path: Path,
) -> None:
    ...
    hermes.session_outputs.append(
        _session_output(_session_id="sid_1", reply_target_message_id="om_1")
    )
...
def test_missing_stored_agent_working_dir_blocks_only_task_session(
    tmp_path: Path,
) -> None:
    old_root = tmp_path / "old-agent-root"
    ...
    cfg_current = _config(
        agent_backend=AgentBackendConfig(working_dir="current-agent-root")
    )
    hermes.router_outputs.append(
        {
            "route": "attach_task",
...
def test_group_auto_reply_disabled_downgrades_to_approval(tmp_path: Path) -> None:
    hermes = FakeHermes()
    hermes.session_outputs.append(
        _session_output(reply_target_message_id="om_1", watch_action="close")
    )
    store, service, _ = _service(tmp_path, hermes=hermes)
    ...
    with store.connect() as conn:
        approval = conn.execute(
            "SELECT short_id, status, preview, payload_json FROM approvals"
        ).fetchone()
        notification = conn.execute(
    ...
    assert (
        notify_payload["suggested_reply"] == '<at user_id="ou_ext">Ext</at> reply text'
    )
...
def test_reply_postprocess_success_updates_auto_reply_payload_before_composer(
    tmp_path: Path,
) -> None:
    (tmp_path / "owner_style.md").write_text("# style\n", encoding="utf-8")
    hermes = FakeHermes()
    hermes.session_outputs.append(
        _session_output(reply_target_message_id="om_1", proposed_reply="raw reply")
    )
    hermes.postprocess_outputs.append(
        {"status": "ok", "final_reply": "owner-like reply"}
    )
    cfg = _config(
...
def test_reply_postprocess_humanizer_only_prompt_uses_skill_path(
    tmp_path: Path,
) -> None:
    ...
    hermes.postprocess_outputs.append(
        {"status": "ok", "final_reply": "humanized reply"}
    )
    cfg = _config(
        reply_postprocess=ReplyPostprocessConfig(
            enabled=True,
            humanizer_zh=ReplyPostprocessHumanizerZhConfig(
                enabled=True, skill_path="humanizer.md"
            ),
        ),
        chats={"oc_1": ChatPolicyConfig(auto_reply=True, bot_joined=True)},
    )
...
def test_reply_postprocess_missing_humanizer_creates_owner_review(
    tmp_path: Path,
) -> None:
    hermes = FakeHermes()
    hermes.session_outputs.append(
    ...
    cfg = _config(
        reply_postprocess=ReplyPostprocessConfig(
            enabled=True,
            humanizer_zh=ReplyPostprocessHumanizerZhConfig(
                enabled=True, skill_path="missing-humanizer.md"
            ),
        ),
        chats={"oc_1": ChatPolicyConfig(auto_reply=True, bot_joined=True)},
    )
    ...
    with store.connect() as conn:
        approval = conn.execute(
            "SELECT preview, payload_json FROM approvals"
        ).fetchone()
        send_count = conn.execute(
            "SELECT COUNT(*) AS c FROM actions WHERE kind = 'send_reply'"
        ).fetchone()["c"]
...
def test_reply_postprocess_success_updates_needs_owner_approval_preview(
    tmp_path: Path,
) -> None:
    (tmp_path / "owner_style.md").write_text("# style\n", encoding="utf-8")
    hermes = FakeHermes()
    ...
    hermes.postprocess_outputs.append(
        {"status": "ok", "final_reply": "owner-ish needs owner"}
    )
...
def test_no_reply_does_not_run_reply_postprocess(tmp_path: Path) -> None:
    ...
    hermes.session_outputs.append(
        _session_output(
            answerability="no_reply", proposed_reply="", reply_target_message_id=None
        )
    )
    cfg = _config(reply_postprocess=_postprocess_config())
    store, service, _ = _service(tmp_path, config=cfg, hermes=hermes)
...
def test_reply_postprocess_missing_profile_creates_keep_watching_approval_with_original_candidate(
    ...
    with store.connect() as conn:
        approval = conn.execute(
            "SELECT short_id, preview, payload_json FROM approvals"
        ).fetchone()
        task = conn.execute("SELECT status, closed_at FROM tasks").fetchone()
    payload = json.loads(approval["payload_json"])
    assert approval["preview"] == "raw reply"
...
def test_rejecting_postprocess_failure_approval_keeps_task_and_other_pending_work(
    tmp_path: Path,
) -> None:
    (tmp_path / "owner_style.md").write_text("# style\n", encoding="utf-8")
    hermes = FakeHermes()
    ...
    with store.connect() as conn:
        conn.execute(
            ...
            (old_watch_until, other_approval_id),
        )
        conn.execute(
            "UPDATE tasks SET watch_until = ? WHERE id = ?",
            (old_watch_until, created.task.id),
        )
    with store.connect() as conn:
        ...
        other_short_id = conn.execute(
            "SELECT short_id FROM approvals WHERE id = ?", (other_approval_id,)
        ).fetchone()["short_id"]
    ...
    with store.connect() as conn:
        task = conn.execute(
            "SELECT status, closed_at, watch_until FROM tasks WHERE id = ?",
            (created.task.id,),
        ).fetchone()
        approvals = {
            row["short_id"]: row["status"]
            for row in conn.execute(
                "SELECT short_id, status FROM approvals ORDER BY id"
            ).fetchall()
        }
        other_action = conn.execute(
            "SELECT status FROM actions WHERE id = ?", (other_action_id,)
        ).fetchone()
    assert task["status"] == "watching"
    assert task["closed_at"] is None
...
def test_reply_postprocess_length_guard_routes_to_owner_review(tmp_path: Path) -> None:
    ...
    hermes = FakeHermes()
    hermes.session_outputs.append(
        _session_output(reply_target_message_id="om_1", proposed_reply="short")
    )
    ...
    with store.connect() as conn:
        ...
        audit = conn.execute(
            "SELECT error FROM agent_audits WHERE request_type = 'reply_postprocess'"
        ).fetchone()
    payload = json.loads(approval["payload_json"])
    assert send_count == 0
...
def test_forbidden_hermes_mentions_downgrade_to_approval(tmp_path: Path) -> None:
    ...
    hermes.session_outputs.append(
        _session_output(
            reply_target_message_id="om_1",
            proposed_reply='<at user_id="ou_x">X</at> hi',
        )
    )
    ...
    assert all(
        not command.startswith("/approve") for command in notify_payload["commands"]
    )
...
def test_explicit_group_auto_reply_false_overrides_global_default(
    tmp_path: Path,
) -> None:
    hermes = FakeHermes()
    hermes.session_outputs.append(_session_output(reply_target_message_id="om_1"))
...
def test_unknown_group_auto_reply_disabled_downgrades_to_approval(
    tmp_path: Path,
) -> None:
    hermes = FakeHermes()
    hermes.session_outputs.append(_session_output(reply_target_message_id="om_1"))
    ...
    with store.connect() as conn:
        ...
        notification = conn.execute(
            "SELECT kind, status FROM actions WHERE kind = 'owner_notification'"
        ).fetchone()
        send_count = conn.execute(
            "SELECT COUNT(*) AS c FROM actions WHERE kind = 'send_reply'"
        ).fetchone()["c"]
...
def test_unknown_group_auto_reply_enabled_can_use_user_fallback(tmp_path: Path) -> None:
    ...
    with store.connect() as conn:
        action = conn.execute(
            "SELECT kind, status, payload_json FROM actions WHERE kind = 'send_reply'"
        ).fetchone()
    ...
    cfg = _config(
        chats={
            "oc_1": ChatPolicyConfig(
                auto_reply=True, bot_joined=False, reply_identity="bot"
            )
        }
    )
    store, service, _ = _service(tmp_path, config=cfg, hermes=hermes)
...
def test_resource_dependent_bot_not_joined_creates_owner_notification(
    tmp_path: Path,
) -> None:
    hermes = FakeHermes()
    ...
    _assert_owner_notification_context(
        payload, message_id="om_1", text="hello", sender_name="Ext", chat_id="oc_1"
    )
...
def test_duplicate_ingest_with_blocked_resource_does_not_rerun_task_session(
    tmp_path: Path,
) -> None:
    hermes = FakeHermes()
    cfg = _config(
    ...
    assert [
        (row["stage"], row["status"], row["terminal_reason"]) for row in processing_rows
    ] == [
...
def test_resource_download_failure_blocks_task_session_after_retries(
    tmp_path: Path,
) -> None:
    hermes = FakeHermes()
    cfg = _config(chats={"oc_1": ChatPolicyConfig(auto_reply=True, bot_joined=True)})
    ...
    with store.connect() as conn:
        ...
        processing = conn.execute(
            "SELECT stage, status, attempt_count, terminal_reason FROM message_processing"
        ).fetchone()
        approval_count = conn.execute("SELECT COUNT(*) AS c FROM approvals").fetchone()[
            "c"
        ]
...
def test_task_router_placeholder_can_create_new_task(tmp_path: Path) -> None:
    ...
    hermes.session_outputs.append(
        _session_output(_session_id="sid_2", reply_target_message_id="om_2")
    )
    cfg = _config(chats={"oc_1": ChatPolicyConfig(auto_reply=True, bot_joined=True)})
    store, service, _ = _service(tmp_path, config=cfg, hermes=hermes)
    ...
    with store.connect() as conn:
        ...
        assert (
            conn.execute(
                "SELECT COUNT(*) AS c FROM agent_audits WHERE request_type = 'router'"
            ).fetchone()["c"]
            == 1
        )
        route = conn.execute(
        ...
        task_id = conn.execute(
            "SELECT id FROM tasks WHERE root_message_id = ?", ("om_2",)
        ).fetchone()["id"]
    ...
    assert router_prompt["context_access"]["read_only_uri"].endswith(
...
def test_p2p_single_active_same_task_can_attach_through_task_router(
    tmp_path: Path,
) -> None:
    hermes = FakeHermes()
    hermes.session_outputs.append(
...
def test_p2p_single_active_unrelated_topic_can_create_new_task_through_task_router(
    tmp_path: Path,
) -> None:
    hermes = FakeHermes()
    hermes.session_outputs.append(
    ...
    hermes.router_outputs.append(
        {"route": "new_task", "target_task_id": None, "reason": "new topic"}
    )
    hermes.session_outputs.append(
        _session_output(_session_id="sid_2", reply_target_message_id="om_2")
    )
...
def test_task_router_ignore_records_audit_without_notification(tmp_path: Path) -> None:
    ...
    hermes.session_outputs.append(_session_output(reply_target_message_id="om_1"))
    hermes.router_outputs.append(
        {"route": "ignore", "target_task_id": None, "reason": "not actionable"}
    )
    cfg = _config(chats={"oc_1": ChatPolicyConfig(auto_reply=True, bot_joined=True)})
    store, service, _ = _service(tmp_path, config=cfg, hermes=hermes)
...
def test_task_router_invalid_target_records_ambiguous_audit_and_notification(
    tmp_path: Path,
) -> None:
    hermes = FakeHermes()
    hermes.session_outputs.append(_session_output(reply_target_message_id="om_1"))
    ...
    _assert_owner_notification_context(
        payload, message_id="om_2", text="hello", sender_name="Bob", chat_id="oc_1"
    )
...
def test_task_router_existing_non_candidate_target_is_invalid(
    tmp_path: Path, route: str
) -> None:
    hermes = FakeHermes()
    hermes.session_outputs.append(_session_output(reply_target_message_id="om_1"))
    hermes.session_outputs.append(
        _session_output(_session_id="sid_stray", reply_target_message_id="om_2")
    )
    cfg = _config(chats={"oc_1": ChatPolicyConfig(auto_reply=True, bot_joined=True)})
    store, service, _ = _service(tmp_path, config=cfg, hermes=hermes)
    ...
    stray = service.normalizer.normalize(
        _message(
            "om_stray", chat_id="oc_other", sender_id="ou_stray", sender_name="Stray"
        ),
        default_chat_type="group",
    )
    ...
    with store.connect() as conn:
        ...
        stray_status = conn.execute(
            "SELECT status FROM tasks WHERE id = ?", (stray_task.id,)
        ).fetchone()["status"]
...
def test_task_router_close_task_output_is_schema_failure(tmp_path: Path) -> None:
    ...
    with store.connect() as conn:
        ...
        task_status = conn.execute(
            "SELECT status FROM tasks WHERE id = ?", (first.task.id,)
        ).fetchone()["status"]
        notification = conn.execute(
            "SELECT payload_json FROM actions WHERE kind = 'owner_notification' AND payload_json LIKE ?",
...
def test_task_router_can_reopen_historical_closed_recall_candidate(
    tmp_path: Path,
) -> None:
    hermes = FakeHermes()
    hermes.session_outputs.append(
    ...
    with store.connect() as conn:
        ...
        task = conn.execute(
            "SELECT status, closed_at FROM tasks WHERE id = ?", (created.task.id,)
        ).fetchone()
...
def test_task_router_cannot_attach_historical_closed_recall_candidate(
    tmp_path: Path,
) -> None:
    hermes = FakeHermes()
    hermes.session_outputs.append(
...
def test_task_session_followup_rejects_task_label(tmp_path: Path) -> None:
    ...
    hermes.session_outputs.append(
        _session_output(_session_id="sid_1", reply_target_message_id="om_2")
    )
    store, service, _ = _service(tmp_path, hermes=hermes)
    ...
    payload = json.loads(notification["payload_json"])
    _assert_owner_notification_context(
        payload, message_id="om_2", text="hello", sender_name="Ext", chat_id="ou_chat"
    )


def test_task_session_schema_failure_does_not_persist_session_id(
    tmp_path: Path,
) -> None:
    hermes = FakeHermes()
    invalid_output = _session_output(
        _session_id="sid_bad", reply_target_message_id="om_1"
    )
...
def test_task_session_exception_retries_terminal_without_empty_approval(
    tmp_path: Path,
) -> None:
    hermes = FakeHermes()
...
def test_duplicate_with_routing_audit_but_no_processing_reruns_task_session(
    tmp_path: Path,
) -> None:
    cfg = _config()
    store = SQLiteStore(tmp_path / "agent.sqlite3")
    ...
    first = p2_service.process_raw_message(
        raw, source="p2p", default_chat_type="p2p", run_id="run_1"
    )
    ...
    second = p3_service.process_raw_message(
        raw, source="p2p", default_chat_type="p2p", run_id="run_2"
    )
    ...
    with store.connect() as conn:
        action = conn.execute(
            "SELECT kind, status FROM actions WHERE kind = 'send_reply'"
        ).fetchone()
        processing = conn.execute(
...
def test_approval_inbox_approves_pending_request_and_advances_checkpoint(
    tmp_path: Path,
) -> None:
    store = SQLiteStore(tmp_path / "agent.sqlite3")
    cfg = _config()
    ...
    with store.connect() as conn:
        approval_short_id = conn.execute("SELECT short_id FROM approvals").fetchone()[
            "short_id"
        ]
    ...
    with store.connect() as conn:
        ...
        command = conn.execute(
            "SELECT status FROM approval_commands WHERE message_id = ?", ("om_cmd",)
        ).fetchone()
        action = conn.execute(
            "SELECT kind, status, target_message_id FROM actions WHERE kind = 'send_reply'"
        ).fetchone()
        task = conn.execute(
            "SELECT status, closed_at FROM tasks WHERE id = ?", (created.task.id,)
        ).fetchone()
...
def test_approval_inbox_fetch_failure_does_not_advance_checkpoint(
    tmp_path: Path,
) -> None:
    store = SQLiteStore(tmp_path / "agent.sqlite3")
    cfg = _config()
    ...
    store.set_checkpoint(
        "approval_inbox", {"last_success_at": "2026-06-22T10:00:00+08:00"}
    )
...
def test_approval_inbox_processes_existing_owner_command_message(
    tmp_path: Path,
) -> None:
    store = SQLiteStore(tmp_path / "agent.sqlite3")
    cfg = _config()
    ...
    with store.connect() as conn:
        ...
        action = conn.execute(
            "SELECT status, target_message_id FROM actions WHERE kind = 'send_reply'"
        ).fetchone()
    assert command["status"] == "applied"
    assert approval["status"] == "approved"
    assert action["status"] == "pending"
...
def test_approve_task_shortcut_single_pending_sets_task_watching(
    tmp_path: Path,
) -> None:
    store = SQLiteStore(tmp_path / "agent.sqlite3")
    cfg = _config()
...
def test_failed_approval_command_rolls_back_state_changes(tmp_path: Path) -> None:
    ...
    with store.connect() as conn:
        approval = conn.execute(
            "SELECT status FROM approvals WHERE short_id = ?", ("a_bad",)
        ).fetchone()
        command = conn.execute(
            "SELECT status FROM approval_commands WHERE message_id = ?", ("om_bad_cmd",)
        ).fetchone()
...
def test_approval_command_with_extra_text_fails_without_consuming_approval(
...
def test_approval_creation_rolls_back_when_owner_notification_fails(
    tmp_path: Path,
) -> None:
    ...
    try:
        ...
        raise AssertionError(
            "owner notification failure did not abort approval creation"
        )
...
def test_approval_command_expires_overdue_before_resolving_command(
    tmp_path: Path,
) -> None:
    store = SQLiteStore(tmp_path / "agent.sqlite3")
...
def test_dispatchable_action_reads_do_not_expire_overdue_approval(
    tmp_path: Path,
) -> None:
    store = SQLiteStore(tmp_path / "agent.sqlite3")
    cfg = _config(lifecycle=LifecycleConfig(approval_timeout_hours=1))
...
def test_approval_expiry_cancels_related_pending_send_and_keeps_task_watching(
    tmp_path: Path,
) -> None:
    store = SQLiteStore(tmp_path / "agent.sqlite3")
    cfg = _config(lifecycle=LifecycleConfig(approval_timeout_hours=1))
    ...
    with store.connect() as conn:
        conn.execute(
            "UPDATE approvals SET expires_at = ? WHERE id = ?",
            ("2026-06-22T09:00:00+08:00", approval_id),
        )
...
def test_approval_expiry_keeps_pending_send_for_other_approved_approval(
    tmp_path: Path,
) -> None:
    store = SQLiteStore(tmp_path / "agent.sqlite3")
    cfg = _config(lifecycle=LifecycleConfig(approval_timeout_hours=1))
    ...
    with store.connect() as conn:
        approvals = {
            ...
            for row in conn.execute(
                "SELECT id, status, resolved_at FROM approvals"
            ).fetchall()
        }
        action = conn.execute(
            "SELECT status FROM actions WHERE id = ?", (send_action["id"],)
        ).fetchone()
        task = conn.execute(
            "SELECT status, closed_at FROM tasks WHERE id = ?", (created.task.id,)
        ).fetchone()
...
def test_concrete_reject_closes_task_and_prevents_other_pending_approval_revival(
    tmp_path: Path,
) -> None:
    store = SQLiteStore(tmp_path / "agent.sqlite3")
    cfg = _config()
    ...
    pending_action_id = store.create_send_reply_action(
        ...
        payload={
            "reply_target_message_id": "om_followup",
            "text": "pending reply",
            "identity": "user",
        },
    ...
    with store.connect() as conn:
        ...
        approvals = {
            row["id"]: row["status"]
            for row in conn.execute(
                "SELECT id, status FROM approvals ORDER BY id"
            ).fetchall()
        }
        pending_action = conn.execute(
            "SELECT status FROM actions WHERE id = ?", (pending_action_id,)
        ).fetchone()
...
def _assert_pending_approval_options(
    payload: dict[str, Any], *, task_short_id: str
) -> None:
    ...
    assert {item["approval_id"] for item in pending_approvals} == set(
        payload["pending_approval_ids"]
    )
...
def test_approve_task_shortcut_multiple_pending_approvals_creates_owner_notification(
    tmp_path: Path,
) -> None:
...
def test_reject_task_shortcut_multiple_pending_approvals_creates_owner_notification(
    tmp_path: Path,
) -> None:
    store, task_short_id = _task_with_two_pending_approvals(tmp_path)
    ...
    with store.connect() as conn:
        ...
        task = conn.execute(
            "SELECT status FROM tasks WHERE short_id = ?", (task_short_id,)
        ).fetchone()
        command = conn.execute(
            "SELECT status FROM approval_commands WHERE message_id = ?",
            ("om_reject_conflict",),
...
def test_approve_task_shortcut_mixed_pending_approval_kinds_creates_owner_notification(
    tmp_path: Path,
) -> None:
    store = SQLiteStore(tmp_path / "agent.sqlite3")
    cfg = _config()
    ...
    assert len(payload["pending_approval_ids"]) == 2
    pending_approvals = {
        item["approval_id"]: item for item in payload["pending_approvals"]
    }
    assert pending_approvals["a_tool"]["kind"] == "tool_action"
    assert pending_approvals["a_tool"]["commands"] == [
        "/approve a_tool",
        "/reject a_tool",
    ]


def test_send_command_multiple_pending_approvals_creates_owner_notification(
    tmp_path: Path,
) -> None:
    store = SQLiteStore(tmp_path / "agent.sqlite3")
    cfg = _config()
...
def test_send_command_without_pending_approval_uses_task_root_message(
    tmp_path: Path,
) -> None:
    store = SQLiteStore(tmp_path / "agent.sqlite3")
    cfg = _config()
    ...
    with store.connect() as conn:
        approval = conn.execute("SELECT status, payload_json FROM approvals").fetchone()
        action = conn.execute(
            "SELECT status, target_message_id, payload_json FROM actions WHERE kind = 'send_reply'"
        ).fetchone()
        task = conn.execute(
            "SELECT status FROM tasks WHERE id = ?", (created.task.id,)
        ).fetchone()
...
def test_send_command_creates_action_for_single_pending_approval(
    tmp_path: Path,
) -> None:
    store = SQLiteStore(tmp_path / "agent.sqlite3")
    cfg = _config()
    ...
    with store.connect() as conn:
        approval = conn.execute("SELECT status FROM approvals").fetchone()
        action = conn.execute(
            "SELECT status, payload_json FROM actions WHERE kind = 'send_reply'"
        ).fetchone()
        task = conn.execute(
            "SELECT status FROM tasks WHERE id = ?", (created.task.id,)
        ).fetchone()
...
def test_concrete_approve_conflict_does_not_consume_approval(tmp_path: Path) -> None:
    ...
    with store.connect() as conn:
        approval_ids = [
            row["short_id"]
            for row in conn.execute(
                "SELECT short_id FROM approvals ORDER BY id"
            ).fetchall()
        ]
...
def test_send_command_active_action_conflict_rolls_back_temporary_approval(
    tmp_path: Path,
) -> None:
    store = SQLiteStore(tmp_path / "agent.sqlite3")
    cfg = _config()
...
def test_send_command_active_action_conflict_keeps_task_watching(
    tmp_path: Path,
) -> None:
    store = SQLiteStore(tmp_path / "agent.sqlite3")
    cfg = _config()
...
def test_send_command_reuses_failed_auto_reply_action_and_idempotency_key(
    tmp_path: Path,
) -> None:
    store = SQLiteStore(tmp_path / "agent.sqlite3")
    cfg = _config()
    ...
    with store.connect() as conn:
        before = conn.execute(
            "SELECT idempotency_key FROM actions WHERE id = ?", (action_id,)
        ).fetchone()
    ...
    with store.connect() as conn:
        ...
        approval = conn.execute(
            "SELECT status FROM approvals WHERE id = ?", (actions[0]["approval_id"],)
        ).fetchone()
...
def test_send_command_sending_action_conflict_rolls_back_temporary_approval(
    tmp_path: Path,
) -> None:
    store = SQLiteStore(tmp_path / "agent.sqlite3")
    cfg = _config()
...
def test_send_command_preserves_owner_final_reply_format(tmp_path: Path) -> None:
    ...
    with store.connect() as conn:
        ...
        stored_command = conn.execute(
            "SELECT command FROM approval_commands"
        ).fetchone()
    approval_payload = json.loads(approval["payload_json"])
    action_payload = json.loads(action["payload_json"])
