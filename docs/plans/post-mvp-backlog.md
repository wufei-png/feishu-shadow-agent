...
状态截至 2026-09-20。编号是默认顺序；会话 06 可在会话 01 后与现场验收并行安排，07 必须等 06，08 必须等 07。状态和证据在完成该会话时更新。
...
| [04 · 运行时现场验收](post-mvp/04-runtime-acceptance.md) | S4 ingest 追赶与 S5 离群/重入时间线 | 03、获准测试 chat | S5 部分通过；S4 现场分页失败待诊断（见 `docs/operations/runtime-acceptance.md`） |
...
- S4 ingest、S5 runtime membership、S6 路由、S7 统一处理重试、S8 owner background 已实现。现场高流量与离群/重入尚无本轮验收；会话 04 仍负责真实 test-chat 的离群/重入时间线。
